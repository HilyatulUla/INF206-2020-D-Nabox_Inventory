...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-gradient-warning">
        <h5 class="modal-title text-white h3" id="exampleModalLabel"><i class="fas fa-plus"></i> Tambah Barang</h5>
      ...
      <div class="modal-body">
          ...
          @csrf
          <input type="hidden" name="jenis" value="makanan" required>
  ...
  <p class="h1 text-white mt-3 text-warning"> <i class="fas fa-hamburger  ml-2"></i> Daftar Barang
...
<div class="row mt-4 shadow">
    ...
    <div class="col-md">
        ...
        <tbody>
        @foreach($barangdag as $i => $item)
            <tr>
            <td style="text-align: center;">{{++$i}}</td>
            <td><img src="{{asset('img/'.$item->gambar)}}" alt="barangdag" class="img-fluid"></td>
            <td>{{$item->nama}}</td>
            <td>@currency($item->harga)</td>
            <td>
                <a href="{{url('/barang/'.$item->id.'/edit')}}" class="btn btn-primary btn-sm px-3 py-2 mb-2"><i class="fas fa-edit"></i> </a>
